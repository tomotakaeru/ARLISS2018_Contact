...
fs_txt_place="/home/pi/ARLISS/fs_para.txt"
...
def log_lux():
    ...
    with open(txt_place, 'a') as f:
        f.write(addTime(str(lux)))

def log_sentence(value):
    with open(txt_place, 'a') as f:
        f.write(addTime(value))

def log_sentence_fs(value):
    with open(fs_txt_place, 'a') as f:
        f.write(addTime(value))
...
if __name__ == "__main__":
    ...
    log_sentence("operation_start")
    log_sentence_fs("start")
    while True:
        time.sleep(0.1)
        sensor.getlux()
        log_lux()
        if lux > 700:
            print("blight_lux: {0:f}".format(lux))
            log_sentence("blight_lux: {0:f}".format(lux))
            print("START")
            with open(judge_txt_place, "a") as f:
                f.write("\tSTART_BY_LUX\r\n")
            log_sentence_fs("lux_or_pressure_end")
            break
